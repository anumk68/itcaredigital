        ...
        document.getElementById('auth-form').addEventListener('submit', async function (e) {
            ...
            try {
                if (isForgotPassword) {
                    ...
                    // Normal login/register - Stage 2: Verify OTP
                    const route = isRegister ? "{{ route('verify.register') }}" : "{{ route('verify.login') }}";
                    const response = await fetch(route, {
                        method: "POST",
                        headers: {
                ...
                submitBtn.disabled = false;
                submitBtn.innerHTML = document.getElementById('submit-btn').innerText;
            }
        });
        function showError(message) {
            const messageEl = document.getElementById('message');
    ...
    <!-- <div class="container">
        <div class="col-md-6 mx-auto">
            <div class="login-container">
                ...
                <p> <a href="{{ route('register.frontend.store') }}">Register</a>
                </p>
            </div>
        </div>
    </div> -->
